...
10. **Q：静态 CMOS 反相器的器件参数如下：
    ...
    1. **请确定 $W_p/W_n$ 的比值，使反相器的开关阈值 $V_M=1.25\text{V}$（注：忽略沟长调制作用影响）；**
        ...
        $$
        \frac{(W/L)_p}{(W/L)_n}=\frac{k_n^{\prime}V_{DSATn}(V_M-V_{Tn}-V_{DSATn}/2)}{k_p^{\prime}V_{DSATp}(V_{DD}-V_M+V_{Tp}+V_{DSATp}/2)}=\frac{115\times 10^{-6}\times0.63\times(1.25-0.4-0.63/2)}{(-30)\times10^{-6}\times(-1)\times[2.5-1.25+(-0.4)+(-1)/2]}=3.7
        $$
...
17.  **Q：一个由两个相反类型的锁存器组成的寄存器（如下图所示），驱动端 $D$ 由一个反相器所驱动，所有的交叉偶合反相器的 $W_p/W_n=2$，并且 $W_p$ 和 $W_n$ 均是 $0.25\mu\text{m}$ 工艺的晶体管最小宽度尺寸，**
    ...
    2. **由于该寄存器的时钟开关采用的是传输管结构，请确定传输管的最小尺寸限制，确保能够在输入端 $D$ 为“$0$”时，能够将其写入寄存器中（忽略沟道调制且如果发生漏电流饱和则为速度饱和，晶体管的其它参数参考教材表 3.2）；**
        ...
        当 B、A 试图将 $a$ 点电压下拉时，反馈回路反相器中的 PMOS 试图将 $a$ 点电压上拉保持，从而存在竞争关系。因此，想要将“$0$”写入寄存器必须满足 A、B 的驱动强度大于 C。首先，求反相器的开关阈值电压 $V_M$，有

        $$r=\frac{k_pV_{DSATp}}{k_nV_{DSATn}}=0.828$$

        因此 $V_M=0.453V_{DD}$。

        假设 $a$ 点电压为 $V_M$，且 A、B 相同，则 $V_D=V_M/2$，此时所有晶体管源漏电流相等，于是有
        ...
        假设 $V_{DD}=2.5\text{V}$，代入数据，可得 $\frac{(W/L)_p}{(W/L)_n}=6.30$，因此 $(W/L)_n/(W/L)_p>1/6.3=0.159$
    ...
    4. **如果传输管的延时是 $t_T$，反相器的延时是 $t_{INV}$，给出寄存器建立时间 $t_{SU}$，传输延迟 $t_{cq}$ 和维持时间 $t_{hold}$。**
        ...
        答：$CLK=1$ 时，第一级传输管导通，在 $CLK=1\to 0$ 前，$D$ 要传播到 $b$ 点。因此 $t_{SU}=t_T+t_{INV}$。$CLK=0$ 时，信号从 $b$ 传播到 $Q$ 的时间即为传输延迟 $t_{cq}=t_T+t_{INV}$。由于第一级传输管在 $CLK=0$ 时立即断开，因此 $D$ 的值可以立即改变，从而 $t_{hold}=0$。 
...
19. **Q：下图是一个 $+1$ 循环计数器电路，循环计数器逻辑块的输入为 $4$ 比特 `IN[3:0]`，输出为 $4$ 比特 `OUT[3:0]`，比特位 `[3]` 表示最高位，比特位 `[0]` 表示最低位，循环计数器的最大计数为 $9(1001)$，当输入 `IN[3:0]` 等于 $9$ 时，输出 `OUT[3:0]` 等于 $0$**
    ...
    3. **如果 `IN[3:0]` 的每一个比特位的负载电容是 $5\text{fF}$，`OUT[3:0]` 的每一个比特位的负载电容是 $4\text{fF}$，电路时钟频率为 $250\text{MHz}$，$V_{DD}=2.5\text{V}$，计算该电路的动态功耗。**
        ...
        其中，$\alpha_{0\to1}=\Sigma_0^3\alpha_{IN[i]}=0.9$，$C_L=4\text{fF}+5\text{fF}=9\text{fF}$，从而 $P=12.7\mu\text{W}$。
